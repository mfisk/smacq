 ...
 *
 * IDENTIFICATION
 *	  $Header: /home/cvs/flow/libsmacq/sql/datum.c,v 1.1 2002/10/30 17:39:26 wbarber Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
datumGetSize(Datum value, bool typByVal, int typLen)
{
	...
	else
	{
		if (typLen == -1)
		{
			...
			if (!PointerIsValid(s))
				fprintf(stderr, "datumGetSize: Invalid Datum Pointer");
			size = (Size) VARSIZE(s);
		}
		else
		{
