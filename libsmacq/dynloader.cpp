...
static void * smacq_try_dlfindsym_preload(char * format, char * sym) {
    ...
    fprintf(stderr, "Looking for symbol %s in preloaded symbol table\n", trysym);
...
void * smacq_find_module(lt_dlhandle* gmodulep, char * envvar, char * envdefault, char * modformat, char * symformat, char * sym) {
    ...
    // Try python symbols second.  If the symbol is a python smacq module, smacq_find_module will be
    // called again to find the pymod module, which will be used as a wrapper for the python smacq module
    // itself.  The information needed to determine which python smacq to wrap will be passed to the
    // pymod module in the context structure.
    modtable = smacq_find_pymodule(gmodulep, envvar, envdefault, modformat, symformat, sym);
    if (modtable) {
        fprintf(stderr, "Found %s in pymodules, getting pymod to wrap it.\n", sym);
        return modtable;
    }
